...
def generate_prompt(age_group, job, household, region, scenario):
    ...
    **고객 프로필:**
    ...
    2. 선택된 세부 시나리오에 대한 실용적 팁 제공 (12단어 이내)
    3. 연령대/직업군별 특화 팁 (8단어 이내)
    4. 지역 특성을 반영한 맞춤 정보 (8단어 이내)
    5. 안전과 대비의 중요성 강조 및 관련 보험 상품 소개 (12단어 이내)
    6. AI 생성 콘텐츠 명시 및 개인정보 보호 관련 안내 (8단어 이내)

    전체 내용은 60단어 이내로, 선택된 시나리오를 중심으로 고객 프로필에 맞춰 균형 잡힌 정보를 반드시 고객에게 바로 보낼 수 있는 정도의 메시지 포맷으로 제공해주세요. 

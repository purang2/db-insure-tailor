...
def generate_prompt(age_group, job, household, region, scenario):
    ...
    반드시 보험 아이템 소개, 보험 권유 두 가지는 지양해주고, 고객 프로필의 잠재적 리스크 (예: 60대=졸음운전)에 초점을 맞춰 데이터를 제공해주세요.
    ...
    반드시 전체 카드뉴스 내용은 120단어를 넘지 않도록 해주세요. 
    결과물은 사용자가 보게 됩니다 따라서 절대로 포맷을 오픈하지 말고 카드뉴스 최종 텍스트만 생성해주세요. 레이아웃을 모바일 환경에 특화하여 잘 고려해주세요.
...
st.title('[DB] 고객맞춤형 카드뉴스 문구 생성 AI')
